 ...
 *
 * A new PawBall's card artwork, generated by fal.ai's
 ...
 * and home region.
 *
 * Called from capture.pipeline.ts's createPawBall() *after* the PawBall
 * document already exists (with the original capture photo as its
 * artwork) — this call updates it in place rather than being part of the
 * initial create, so a slow or failed fal.ai call never delays or blocks
 * the PawBall from existing with a usable artwork.
 *
...
function buildPrompt(input: ArtworkGenInput): string {
  return (
    `${input.breed} cat as fantasy ${input.fantasyClass} with ${input.element} elemental magic, ` +
    `glowing ${input.coatColor} fur, ${input.regionName} background, premium trading card ` +
    `illustration, painterly style, highly detailed, magical`
  );
}
