 ...
 *   - `loreText`   — the creature's origin lore paragraph. Generated by a
 *                     real Gemini call (modules/lore/lore.gemini.ts), with a
 *                     seeded-template fallback if Gemini is unavailable.
 *                     Weekly *updates* are separate `memories` docs; this is
 *                     the fixed origin text.
 *   - `personality`— a 1-2 sentence behavioural blurb from the same Gemini
 *                     call as `loreText`.
